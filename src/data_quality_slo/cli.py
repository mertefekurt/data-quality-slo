...
from data_quality_slo import __version__
from data_quality_slo.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from data_quality_slo.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='data-quality-slo',
        description=(
                        'Check data quality SLO manifests for freshness, completeness, and owner '
                        'fields.'
                    ),
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    parser.add_argument("input", nargs="?", type=Path, help='data quality manifest')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"data-quality-slo: error: {exc}", file=sys.stderr)
        return 1
